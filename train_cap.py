...
def update_train_loader(dataset, train_samples, updated_label, height, width, batch_size, re, workers,
    ...
    target_train_loader = DataLoader(
        UnsupervisedTargetPreprocessor(new_train_samples, root=osp.join(dataset.target_images_dir, dataset.target_train_path),
                                         num_cam=dataset.target_num_cam, transform=train_transformer, has_pseudo_label=True),
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        updated_label, init_intra_id_feat = img_association(model, propagate_loader, min_sample=4,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_bnneck', action='store_true')
