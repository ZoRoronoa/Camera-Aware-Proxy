...
def img_association(network, propagate_loader, min_sample=4, eps=0,
    ...
    for glab in np.unique(global_labels):
        ...
        # new_file_names.append(file_names[idx])

    new_features = np.array(new_features)
    new_cams = np.array(new_cams).squeeze()
    # new_file_names = np.array(new_file_names).squeeze()
    
    # np.save("new_cams.npy", new_cams)
    # np.save("filenames.npy", file_names)
    del features, all_cams
    # np.save(osp.join("real_labels.npy"), real_labels)
    # compute distance W
    ...
    # _, updated_label = dbscan(W, eps=0.6, min_samples=8, metric='precomputed', n_jobs=8)    
